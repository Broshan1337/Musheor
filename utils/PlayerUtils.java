...
import net.minecraft.class_2661;  // ChatMessageC2SPacket (command)
import net.minecraft.class_304;   // KeyBinding
import net.minecraft.MinecraftClient;   // MinecraftClient
import net.minecraft.class_5250;  // Text component
import net.minecraft.class_7472;  // ChatMessageC2SPacket

public class PlayerUtils {
    private static final MinecraftClient mc = MinecraftClient.method_1551(); // MinecraftClient.getInstance() — was: XrtBzLhuwKI
    ...
    /** Sends a chat message with the "[Musheor]: " prefix. */
    public static void sendChatMessage(String message) { // was: J2pm2c07elEb5G(String)
        assert (PlayerUtils.mc.field_1724 != null);
        class_5250 text = class_2561.method_43470("[Musheor]: " + message); // Text.literal
        PlayerUtils.mc.player.field_3944.method_52781(new class_2661((class_2561) text)); // sendCommand
    }
    ...
    public static void setSneaking(boolean sneaking) { // was: jWrhVf2psx(boolean)
        PlayerUtils.mc.options.field_1832.method_23481(sneaking); // options.sneakKey.setPressed
    }

    /** Disables the HighwayBuilder module entirely. */
    public static void disableHighwayBuilder() { // was: xynAsOKhN7t
        HighwayBuilder.cleanup(); // was: V2mbWoNZftH0t
    ...
    public static void enableFreeLookMode() { // was: Gd2ks78ySQq40
        if (PlayerUtils.mc.field_1724 != null && PlayerUtils.mc.field_1687 != null) {
            ...
            PlayerUtils.mc.player.method_36457(15.0f); // setPitch
                ...
                try {
                    ...
                    switch (SwitchBootstraps.typeSwitch("typeSwitch",
                            new Object[]{Integer.class, Double.class, String.class, Boolean.class,
                                         ItemStack.class, Block.class, Enum.class},
                            value, typeCase)) {
                        ...
                        case 4: ((ItemSetting) setting).set((Object)(ItemStack) value); break;
    ...
    public static void alignLookToHighway() { // was: Bocqo9ajQ
        assert (PlayerUtils.mc.field_1724 != null);
        PlayerUtils.mc.player.method_36457(30.0f); // setPitch
        WorldUtils.Direction8 dir = HighwayBuilder.getDirection(); // was: kLIvClyeu
        PlayerUtils.mc.player.method_36456(WorldUtils.Direction8.toYaw(dir)); // setYaw
    }
     ...
     */
    public static void startGatherItem(ItemStack item, boolean unused) { // was: jOdDDFXSeWl4(Item,boolean)
    ...
    /** No-op stub (was usJLOV0subXO3(boolean)). */
    ...
    public static void setKeyState(class_304 key, boolean pressed) { // was: jOdDDFXSeWl4(KeyBinding,boolean)
        key.method_23481(pressed); // setPressed
    ...
    public static void applyStrafing() { // was: quFLaIBj1UQn6g
        if (PlayerUtils.mc.field_1724 == null || PlayerUtils.mc.field_1687 == null) return;
        ...
        if (state.getDirection() == WorldUtils.Direction8.NORTH // was: vSouwXdh7
                || state.getDirection() == WorldUtils.Direction8.SOUTH) { // was: Q5FUNqd0ALfl
            double playerX = VersionHelper.get().getPlayerPos().method_10216(); // getX
            if (state.getDirection() == WorldUtils.Direction8.NORTH) {
                if (playerX > state.getAlignX() + 0.13) strafeLeft  = true; // was: JDwgf5
        ...
        } else if (state.getDirection() == WorldUtils.Direction8.WEST // was: S8iuqKQCrJM02b
                || state.getDirection() == WorldUtils.Direction8.EAST) { // was: v5UhyO9eEd7n
            double playerZ = VersionHelper.get().getPlayerPos().method_10215(); // getZ
            if (state.getDirection() == WorldUtils.Direction8.WEST) {
                if (playerZ > state.getAlignZ() + 0.13) strafeLeft  = true; // was: yyKeW1d7hG
        ...
        PlayerUtils.setKeyState(PlayerUtils.mc.options.field_1849, strafeRight);  // options.leftKey
        PlayerUtils.setKeyState(PlayerUtils.mc.options.field_1913, strafeLeft);   // options.rightKey
    }
    ...
    public static void alignWithBaritone() { // was: uFghvYncEwFBHmJL
        HighwayState state = HighwayState.getInstance();
        if (state.getLavaTargetBlock() != null) return; // was: HBAiI3pyGGGxpI2b
        assert (PlayerUtils.mc.field_1724 != null);
        if (state.getAlignStartX() != 0 && state.getHighwayY() != 0  // was: Icks58Pk4vQH3, KaWPzeyl1xVKHWo
                ...
                && (Math.abs(PlayerUtils.mc.player.getX()) % Math.abs(state.getAlignStartX()) > 1
                 || Math.abs(PlayerUtils.mc.player.getZ()) % Math.abs(state.getAlignStartZ()) > 1)) { // was: A02ApsqZGj
            GoalBlock goal = new GoalBlock(
                state.getAlignStartX().intValue(),
                state.getHighwayY().intValue(),
                state.getAlignStartZ().intValue());
            PathingHelper.setBaritoneGoal((Goal) goal); // was: jOdDDFXSeWl4
    ...
    public static void alignWithBaritoneXZ() { // was: HUYtvX
        ...
        if (PlayerUtils.mc.player.getZ() != 0
                ...
                && (offset = PlayerUtils.mc.player.getZ() - state.getAlignZ()) > 0.5) { // getZ
            GoalBlock goal = new GoalBlock(
        ...
        if (PlayerUtils.mc.player.getX() != 0
                ...
                && (offset = PlayerUtils.mc.player.getX() - state.getAlignX()) > 0.5) { // getX
    ...
    /**
     * Sends a teleport-request message to the given player using a truncated UUID
     * as a verification token. Uses musheor.plus.PlusPlayerUtils via reflection if
     * available (for IRC-based sending), otherwise falls back to /msg.
     */
    public static void sendTeleportMessage(String targetPlayer, int tokenLength) { // was: jOdDDFXSeWl4(String,int)
        ...
        try {
            ...
            PlayerUtils.mc.player.field_3944.method_52787( // networkHandler.sendPacket
                (class_2596) new class_7472("msg " + targetPlayer + " " + command)); // ChatMessageC2SPacket
        }
    }
}
